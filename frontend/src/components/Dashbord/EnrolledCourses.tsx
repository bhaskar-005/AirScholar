  ...
  return (
    <div className="flex flex-col gap-5 my-[20px] mb-[100px]">
      ...
      <div className="flex  flex-col sm:gap-8 gap-3">
        {/* course card */}
        {noCourse && (
          <div className="text-black text-[17px] font-[600]">
            You are not currently enrolled in any courses.
          </div>
        )}

        {courses && courses.length > 1
          ? [...courses].reverse().map((item: any, index) => (
              <Link key={index} to={`/viewCourse/${item._id}`}>
                <div
                  key={index}
                  className="flex flex-row border items-center rounded-[3px]"
                >
                  ...
                  <div className="flex flex-col gap-0 w-full sm:mx-10 mx-2">
                    <div className="flex flex-col">
                      ...
                      <div className="flex flex-col sm:gap-2 gap-1">
                        <div>
                          <h2 className="lg:text-[24px] sm:text-[18px] text-[12px] font-[500]">
                            {item.couresName.slice(0, 40)}..
                          </h2>
                          <p className="sm:block hidden font-[400] lg:text-[14px] text-[13px] text-gray-500">
                            {item.courseDescription.slice(0, 90)}..
                          </p>
                        </div>
                        <p className="text-gray-600 lg:text-[13px] sm:text-[15px] text-[10px] font-[500]">
                          Completed Lessons:{" "}
                          <span className="text-black">
                            0 of{" "}
                            {`${calculateTotalSectioin(item.courseContent)}`}
                          </span>{" "}
                        </p>
                      </div>
                    </div>
                    <div className="sm:mt-[10px] mt-[2px] flex justify-between">
                      <div className="w-full sm:block hidden">
                        <ProgressBar completed={0} bgColor="#2563EB" />
                      </div>
                    </div>
                  </div>
                </div>
